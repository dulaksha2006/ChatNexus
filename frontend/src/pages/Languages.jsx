...
function LangTextsEditor({ code, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-950 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col animate-slide-up">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {loading ? <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-brand-400" /></div> : <>
            <p className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest">System Messages</p>
            ...
            {commands.length > 0 && <>
              <div className="pt-2 border-t border-white/5">
                <p className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest mb-3">Command Responses</p>
                {commands.map(cmd => (
                  <div key={cmd.id} className="mb-4">
                    <label className="block text-xs font-medium text-zinc-400 mb-1.5">
                      <code className="text-brand-400 text-[10px]">/{cmd.command}</code> — {cmd.label}
                    </label>
                    <textarea className="input-field resize-none text-sm" rows={2}
        ...
        <div className="px-5 py-4 border-t border-white/5 flex gap-3 justify-end">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-white/10 text-sm text-zinc-400 hover:text-white transition-all">Cancel</button>
  ...
  return (
    <Layout title="Languages">
      ...
      <div className="flex justify-between items-center mb-6">
        <p className="text-sm text-zinc-500">{langs.length} language{langs.length !== 1 ? 's' : ''}</p>
      ...
      {showAdd && (
        <div className="bg-zinc-950 border border-white/10 rounded-xl p-4 mb-4 animate-slide-up">
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Code (e.g. en)</label>
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Flag emoji</label>
              <input className="input-field text-sm" placeholder="🇺🇸"
                value={newLang.flag} onChange={e => setNewLang(p => ({ ...p, flag: e.target.value }))} />
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowAdd(false)} className="px-3 py-1.5 rounded-lg border border-white/10 text-xs text-zinc-400 hover:text-white transition-all">Cancel</button>
      ...
      <div className="bg-zinc-950 border border-white/5 rounded-xl overflow-hidden">
        {loading ? (
          <div className="flex justify-center py-12"><Loader2 className="w-5 h-5 animate-spin text-brand-400" /></div>
        ) : langs.length === 0 ? (
          <div className="flex flex-col items-center py-12 text-center">
            <Globe className="w-7 h-7 text-zinc-800 mb-2" />
            <p className="text-sm text-zinc-500">No languages yet</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {langs.map(l => (
                <tr key={l.code} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <code className="text-xs text-zinc-500 bg-zinc-900 px-2 py-0.5 rounded">{l.code}</code>
                  </td>
                  <td className="px-5 py-3.5">
                    <button onClick={() => toggleActive(l)}
                      className={clsx('text-xs px-2.5 py-1 rounded-full border transition-all',
                        l.active ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20 hover:bg-emerald-500/20'
                                 : 'text-zinc-600 bg-zinc-500/10 border-zinc-700 hover:text-white')}>
