  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer select-none">
      <div onClick={onChange}
        className={clsx(
          'w-8 h-4 rounded transition-all relative',
          checked ? 'bg-white' : 'bg-zinc-700'
        )}>
        <span className={clsx(
          'absolute top-0.5 w-3 h-3 bg-white rounded shadow transition-all',
          checked ? 'left-[18px]' : 'left-0.5'
        )} />
      </div>
      <span className="text-xs text-white">Show in Telegram command list</span>
    </label>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-white mb-1">Command</label>
        ...
        <div>
          <label className="block text-xs text-white mb-1">Button Label</label>
      ...
      <div>
        <label className="block text-xs text-white mb-1">Action</label>
      ...
      {form.action === 'send_menu' && (
        <div>
          <label className="block text-xs text-white mb-1">Which Menu?</label>
      ...
      {form.action === 'custom_text' && (
        <div>
          <label className="block text-xs text-white mb-1">
            ...
            <span className="ml-1 text-white font-normal">(override per-language in Languages)</span>
          </label>
      ...
      <div className="flex justify-end gap-2 pt-1">
        <button onClick={onCancel}
          className="px-3 py-1.5 rounded border border-[#1a1a1a] text-xs text-white hover:text-white transition-all">
          Cancel
        </button>
        <button onClick={onSave} disabled={saving}
          className="px-3 py-1.5 rounded bg-white hover:bg-white text-white text-xs font-medium transition-all disabled:opacity-50 flex items-center gap-1.5">
...
function CommandRow({ cmd, menus, onSave, onDelete }) {
  ...
  if (!editing) return (
    <div className={clsx(
      'flex items-center gap-3 px-4 py-3 rounded border transition-all',
      cmd.active ? 'border-[#1a1a1a] bg-[#0a0a0a]' : 'border-[#1a1a1a] bg-[#0a0a0a] opacity-50'
    )}>
      <GripVertical className="w-4 h-4 text-[#484f58] shrink-0" />
      <code className="text-white text-sm font-mono w-24 shrink-0">/{cmd.command}</code>
      <span className="text-white text-sm flex-1">{cmd.label}</span>
      <div className="flex items-center gap-1.5 shrink-0">
        {cmd.showInCommandList !== false
          ? <span className="text-[10px] text-white bg-white/10 border border-[#1a1a1a]/30 px-1.5 py-0.5 rounded">visible</span>
          : <span className="text-[10px] text-white bg-[#0a0a0a] border border-[#1a1a1a] px-1.5 py-0.5 rounded">hidden</span>}
        <span className="text-xs text-white bg-[#0a0a0a] px-2 py-0.5 rounded border border-[#1a1a1a]">
          {ACTIONS.find(a => a.value === cmd.action)?.label || cmd.action}
        </span>
      </div>
      <div className="flex gap-1.5 shrink-0">
        <button onClick={toggleActive}
          className={clsx('text-xs px-2 py-1 rounded border transition-all',
            cmd.active
              ? 'text-white border-emerald-500/25 bg-white/10 hover:bg-white/20'
              : 'text-white border-[#1a1a1a] hover:text-white')}>
          {cmd.active ? 'On' : 'Off'}
        </button>
        ...
        <button onClick={del} className="p-1.5 rounded text-white hover:text-white hover:bg-white/10 transition-all">
  ...
  return (
    <div className="border border-brand-500/25 bg-white/5 rounded p-4">
      <CommandForm
  ...
  return (
    <div className="border border-[#1a1a1a] bg-[#0a0a0a] rounded overflow-hidden">
      <button onClick={() => setOpen(!open)}
        ...
        <LayoutList className="w-4 h-4 text-white shrink-0" />
        <span className="text-white text-sm font-medium flex-1">{menu.title}</span>
        <span className="text-xs text-white">{items.length} item{items.length !== 1 ? 's' : ''}</span>
      ...
      {open && (
        <div className="border-t border-[#1a1a1a] p-4 space-y-4">
          <div>
            <label className="block text-xs text-white mb-1">Menu Title</label>
            <input className="input-field text-sm" value={title} onChange={e => setTitle(e.target.value)} />
          </div>

          <div className="space-y-2">
            <p className="text-xs font-medium text-white">Items</p>
            {items.map((item, i) => (
              <div key={i} className="flex gap-2 items-start bg-[#0a0a0a] rounded p-3 border border-[#1a1a1a]">
                ...
                <button onClick={() => setItems(p => p.filter((_, idx) => idx !== i))}
                  className="p-1.5 text-white hover:text-white transition-colors shrink-0 mt-0.5">
                  <X className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
            <button onClick={addItem}
              className="w-full py-2 border border-dashed border-[#1a1a1a] rounded text-xs text-white hover:text-white hover:border-white/20 transition-all flex items-center justify-center gap-1.5">
              <Plus className="w-3 h-3" /> Add Item
            </button>
          </div>

          <div className="flex justify-between pt-1">
            <button onClick={del} className="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs text-white bg-white/10 hover:bg-white/20 border border-[#1a1a1a]/30 transition-all">
              <Trash2 className="w-3 h-3" /> Delete
            </button>
            <button onClick={save} disabled={saving}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-white hover:bg-white text-white text-xs font-medium transition-all disabled:opacity-50">
        ...
        {[['commands', Terminal, 'Commands'], ['menus', LayoutList, 'Menus']].map(([key, Icon, label]) => (
          <button key={key} onClick={() => setTab(key)}
            className={clsx('flex items-center gap-2 px-4 py-2 rounded text-sm font-medium transition-all',
              tab === key ? 'bg-white text-white' : 'bg-[#0a0a0a] text-white hover:text-white border border-[#1a1a1a]')}>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <p className="text-sm text-white">{commands.length} command{commands.length !== 1 ? 's' : ''}</p>
          ...
          {showAdd && (
            <div className="bg-[#0a0a0a] border border-[#1a1a1a]/25 rounded p-4 animate-slide-up">
              <p className="text-xs font-semibold text-white mb-3">New Command</p>
              <CommandForm
          ...
          {commands.length === 0 && !showAdd ? (
            <div className="flex flex-col items-center py-16 text-center">
              ...
              <p className="text-sm text-white">No commands yet</p>
            </div>
          ) : (
            commands.map(cmd => (
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <p className="text-sm text-white">{menus.length} menu{menus.length !== 1 ? 's' : ''}</p>
          ...
          {menus.length === 0 ? (
            <div className="flex flex-col items-center py-16 text-center">
              ...
              <p className="text-sm text-white">No menus yet</p>
            </div>
          ) : (
            menus.map(menu => (
