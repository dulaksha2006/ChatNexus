...
/* ─── Status badge config ──────────────────────────────────────────────────── */
const STATUS_CONFIG = {
  free:    { label: 'Online',  color: 'bg-emerald-500', ring: 'border-emerald-500/40', text: 'text-emerald-400' },
  busy:    { label: 'Busy',    color: 'bg-amber-500',   ring: 'border-amber-500/40',   text: 'text-amber-400'   },
  offline: { label: 'Offline', color: 'bg-zinc-600',    ring: 'border-zinc-600/40',    text: 'text-zinc-400'    },
};
...
function GreetingPanel({ user, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-zinc-950 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-white text-sm">Edit My Greeting</h3>
            <p className="text-[11px] text-zinc-500 mt-0.5">Sent automatically when a customer connects to you.</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            <textarea
              className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-zinc-600 outline-none focus:border-brand-500/50 resize-none transition-colors"
              rows={4}
              value={greeting}
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={onClose}
                className="flex-1 py-2 rounded-lg border border-white/10 text-sm text-zinc-400 hover:text-white hover:bg-white/5 transition-all">
                Cancel
              </button>
...
function NewSessionBanner({ session, onDismiss, onOpen }) {
  ...
  return (
    <div className="fixed top-5 right-5 z-50 animate-slide-up">
      <div className="bg-zinc-900 border border-brand-500/40 rounded-2xl p-4 shadow-2xl flex items-start gap-3 max-w-xs">
        ...
        <div className="flex flex-col gap-1.5 shrink-0">
          <button onClick={onOpen}
            className="text-[10px] px-2.5 py-1 rounded-lg bg-brand-500 hover:bg-brand-600 text-white font-medium transition-all">
            Open
          </button>
          <button onClick={onDismiss}
            className="text-[10px] px-2.5 py-1 rounded-lg border border-white/10 text-zinc-500 hover:text-white transition-all">
            Dismiss
          </button>
        </div>
      </div>
    ...
    <div className="relative">
      <button
        ...
        className={clsx(
          ...
          cfg.ring, cfg.text, 'bg-white/5 hover:bg-white/10'
        )}
      >
        {loading
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden z-20 min-w-[130px]">
          {Object.entries(STATUS_CONFIG).map(([key, s]) => (
            <button
              key={key}
              onClick={() => { onChange(key); setOpen(false); }}
              className={clsx(
                'flex items-center gap-2.5 w-full px-3 py-2 text-xs transition-colors hover:bg-white/5',
                currentStatus === key ? s.text + ' font-semibold' : 'text-zinc-400'
              )}
            >
    ...
    <div className="px-4 pb-2 flex gap-1.5 flex-wrap">
      {replies.map((r, i) => (
        <button
          ...
          className="text-[10px] px-2.5 py-1 rounded-lg bg-zinc-800 border border-white/5 text-zinc-400 hover:text-white hover:bg-zinc-700 transition-all truncate max-w-[160px]"
          title={r.text}
        >
          {r.label || r.text}
        </button>
  ...
  return (
    <Layout title="Dashboard">
      ...
      <div className="flex gap-4 h-[calc(100vh-9rem)]">
        ...
        {/* ── Session list ─────────────────────────────────────────────── */}
        <div className="w-60 shrink-0 flex flex-col bg-zinc-950 border border-white/5 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
            <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">Active Sessions</p>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-1 rounded-lg text-zinc-600 hover:text-brand-400 hover:bg-brand-500/10 transition-all"
              >
              ...
              <div className="p-2 space-y-1">
                {sessions.map(s => (
                  <button key={s.id} onClick={() => setActiveId(s.id)}
                    ...
                    {s.customerUsername && (
                      <p className="text-[10px] text-zinc-600 mt-0.5 ml-3.5 truncate">@{s.customerUsername}</p>
                    )}
                    <p className="text-[10px] text-zinc-700 mt-0.5 ml-3.5">
                      {s.createdAt?.toDate
        ...
        {/* ── Chat area ────────────────────────────────────────────────── */}
        <div className="flex-1 flex flex-col bg-zinc-950 border border-white/5 rounded-xl overflow-hidden min-w-0">
          {!activeSession ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              ...
              <button
                onClick={() => setShowGreeting(true)}
                className="mt-5 flex items-center gap-2 text-xs text-zinc-600 hover:text-brand-400 transition-colors px-3 py-1.5 rounded-lg hover:bg-brand-500/10"
              >
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
                {messages.length === 0 && (
                  <p className="text-xs text-zinc-700 text-center pt-4">Session started. Say hello!</p>
                )}
                {messages.map(msg => (
                  <Bubble
